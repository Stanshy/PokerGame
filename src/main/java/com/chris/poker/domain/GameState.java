...
public class GameState {
	...
	private void handleShowdown() {
		...
		if (playerInHnad.size() == 1) {
			...
			List<WinnerInfo> winners = List.of(new WinnerInfo(winner.getName(), pot.getTotalAmount()));
			recordHandHistory(winners);
			pot.awardToWinner(winner);
		} else {
			List<WinnerInfo> winners = distributePot();
			recordHandHistory(winners);
	...
	// 處理底池(含邊池)
	private List<WinnerInfo> distributePot() {
		...
		return pot.distributePots(potWinners);
	}
	...
	// 手牌紀錄
	public void recordHandHistory(List<WinnerInfo> winners) {
		if (currentPhase != GamePhase.SHOWDOWN) {
			return;
		}
		HandHistory history = new HandHistory(handHistories.size() + 1, new ArrayList<>(getPlayersInHand()),
				new ArrayList<>(board.getCards()), pot.getTotalAmount(), winners);
	...
	public List<HandHistory> getHandHistories() {
		return new ArrayList<>(handHistories);
	}
